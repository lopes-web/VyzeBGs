...
const GeneratorWorkspace: React.FC<GeneratorWorkspaceProps> = ({
    ...
    const [localHistory, setLocalHistory] = useState<HistoryItem[]>(() => {
        const saved = localStorage.getItem(`vyze-history-${projectId}`);
    ...
    useEffect(() => {
        if (projectId && localHistory.length > 0) {
            localStorage.setItem(`vyze-history-${projectId}`, JSON.stringify(localHistory));
        }
