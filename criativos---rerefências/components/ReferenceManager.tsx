...
const ReferenceManager: React.FC<ReferenceManagerProps> = ({ 
    ...
    description = "Envie múltiplas referências e descreva o que aproveitar de cada uma (ex: 'Use a iluminação desta', 'Use o cenário daquela')."
}) => {
        ...
        {items.map((item, idx) => (
            <div key={item.id} className="bg-gray-800 rounded-lg p-3 flex gap-4 border border-gray-700 items-start group">
                ...
                <div className="flex-grow">
                    <label className="text-xs text-gray-400 block mb-1">Nota de instrução:</label>
                    <textarea 
                        value={item.description}
                        onChange={(e) => updateDescription(item.id, e.target.value)}
                        placeholder="O que devemos copiar desta imagem?"
