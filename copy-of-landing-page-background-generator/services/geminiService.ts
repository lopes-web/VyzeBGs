// Type declaration for AI Studio environment
declare global {
  interface Window {
    aistudio?: {
...
import { MODEL_NAME, POSITION_PROMPTS, DEFAULT_TREATMENT_PROMPT, OBJECT_TREATMENT_PROMPT, ENHANCE_TREATMENT_PROMPT, BLUR_PROMPT, GRADIENT_PROMPT } from "../constants";
import { SubjectPosition, ReferenceItem, GenerationAttributes, GeneratorMode, ChatMessage } from "../types";
...
export const generateBackground = async (
  ...
  if (!apiKey) throw new Error("API Key not found. Please configure it.");
  ...
  if (userPrompt.trim()) {
    // Add invisible noise to prompt if needed to ensure uniqueness in batch, handled by caller mostly, but good to add context.
  ...
  // Positioning - Only strict for Human/Object. For Enhance, we usually want to respect original unless told otherwise.
  if (mode !== 'ENHANCE') {
...
export const refineImage = async (
  ...
  refineReferenceImages: string[] = [] // Added support for extra references
): Promise<string> => {
  ...
  // Add any supporting images for the edit
...
export const reframeImageForTextLayout = async (
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        imageConfig: {
          aspectRatio: aspectRatio, // Dynamic based on height request, usually 9:16
          imageSize: "2K"
        }
      },
    });
...
export const sendAgentChat = async (
  ...
  try {
    const response = await ai.models.generateContent({
      model: MODEL_NAME, // Using the same powerful model for analysis
      contents: contents,
      config: {
